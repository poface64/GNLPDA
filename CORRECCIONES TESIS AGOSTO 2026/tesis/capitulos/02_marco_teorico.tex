...
La selección de características conserva un subconjunto de variables originales mientras que la extracción construye nuevas dimensiones mediante transformaciones o combinaciones de las variables disponibles. En el contexto de la visualización, ambas estrategias pueden producir representaciones de baja dimensión, aunque difieren en el grado de correspondencia directa que mantienen con las variables originales \cite{zebari2020comprehensive}.

\subsection{Proyecciones discriminantes para la visualización supervisada}

La utilidad de una visualización depende fuertemente del criterio matemático utilizado para proyectar los datos. El Análisis de Componentes Principales (PCA) constituye una técnica ampliamente utilizada para obtener representaciones de baja dimensión. Al proyectar las observaciones sobre sus primeras componentes, se obtiene un mapa que preserva la mayor varianza global de los datos. Sin embargo, al ser un método no supervisado, PCA no incorpora las etiquetas de clase en el proceso de obtención de las componentes \cite{bharadiya2023tutorial}.


En problemas de clasificación, la varianza global no siempre coincide con la separabilidad de las clases. Cuando el objetivo específico es favorecer la separación entre grupos, resulta conveniente emplear proyecciones discriminantes supervisadas, cuyo propósito es orientar la proyección hacia direcciones que favorezcan la separación entre los grupos \cite{nanga2021review}.

Para ilustrar esta diferencia fundamental entre la maximización de la varianza global y la maximización de la separabilidad, la figura \ref{fig:MT1} presenta una comparativa geométrica conceptual. Mientras que PCA proyecta los datos buscando la dirección de mayor dispersión general (ignorando las etiquetas de clase), una proyección discriminante orienta los ejes específicamente para maximizar la separación entre las clases respecto a su dispersión interna \cite{feng2021comparison}.
...
Las representaciones de baja dimensión permiten convertir un conjunto de observaciones definido originalmente en un espacio multidimensional en una geometría que puede ser inspeccionada visualmente. Cuando la representación tiene una, dos o tres dimensiones, es posible recurrir a recursos gráficos que permiten explorar directamente la distribución de las observaciones y su relación con las clases \cite{thrun2020uncovering}.

En un espacio proyectado de baja dimensión, una separación visualmente marcada entre los grupos puede ofrecer evidencia exploratoria acerca de la existencia de una estructura discriminante; sin embargo, dicha representación no constituye por sí misma una prueba estadística de separabilidad ni garantiza desempeño predictivo \cite{acevedo2024geometric}. Su valor radica en proporcionar una representación geométrica que facilite la exploración de la estructura de los datos y la formulación de hipótesis sobre la relación entre las clases, el grado de solapamiento y la presencia de observaciones atípicas, mientras que la evaluación de la capacidad discriminante y del rendimiento predictivo de un clasificador debe sustentarse en métricas cuantitativas \cite{de2025low}.

Además de la posibilidad de inspeccionar visualmente la estructura proyectada, una representación de baja dimensión puede adquirir mayor utilidad interpretativa cuando es posible establecer una relación explícita entre sus dimensiones y las características que intervienen en su construcción \cite{murdoch2019definitions}.
...
En ciertos métodos de reducción dimensional, especialmente aquellos que recurren a transformaciones implícitas del espacio de características, la relación explícita entre los ejes obtenidos y las variables originales tiende a ser menos directa, lo que dificulta la interpretación de la representación en términos de las características iniciales \cite{briscik2023improvement}.
...
El Análisis Discriminante Lineal, comúnmente referido como LDA por sus siglas en inglés, es una técnica estadística multivariante propuesta originalmente por Fisher (1936) \cite{fisher1936use}, cuyo propósito es encontrar combinaciones lineales de las variables originales que permitan favorecer la separación entre dos o más clases. En el contexto de la reducción de dimensionalidad supervisada, LDA busca proyectar los datos desde un espacio original de dimensión ($p$) hacia un subespacio de menor dimensión ($d$), procurando que las observaciones pertenecientes a una misma clase permanezcan compactas, mientras que las clases distintas se encuentren lo más separadas posible \cite{zhao2024linear}.
