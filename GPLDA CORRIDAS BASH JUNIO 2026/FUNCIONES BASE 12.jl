...
function validacion_estadistica_paralela(n_corridas, df_completo, df_particiones, config_e1, config_e2, lambda, modelo)
    
    lista_resultados = Vector{Any}(undef, n_corridas)
    ncol_datos = size(df_completo, 2)
    
    println(">>> Iniciando $n_corridas corridas en paralelo usando $(Threads.nthreads()) hilos...")
    
    # Bucle paralelizado seguro para hilos
    @threads for i in 1:n_corridas
        # Identificar la columna de la corrida actual (ej. "Run_1")
        columna_run = Symbol("Run_$i")
        
        # Crear máscaras booleanas basadas en el archivo de particiones de R
        es_train = df_particiones[!, columna_run] .== "Train"
        es_test  = df_particiones[!, columna_run] .== "Test"
        
        # --- SEGMENTACIÓN ESTRICTA (70/30) ---
        df_train = df_completo[es_train, :]
        df_test  = df_completo[es_test, :]
        
        # Separar variables predictoras (X) y etiquetas (y) para Entrenamiento
        X1_train = select(df_train, Not(ncol_datos))
        y1_train = categorical(String.(string.(df_train[!, ncol_datos])))
        y_train_final = coerce(y1_train, Multiclass)
        
        # Generar el pool polinomial SOLO con el 70% de entrenamiento
        X_pool_train = polexp(X1_train, grado=2)
        
        # --- BÚSQUEDA ESTOCÁSTICA DE GNLPDA ---
        # Solo conoce y opera sobre los datos de entrenamiento (70%)
        res_gpn = ejecutar_gpn_lda(X_pool_train, X1_train, y_train_final, config_e1, config_e2, lambda, modelo)
        
        # --- EVALUACIÓN EXTERNA JUSTA SOBRE EL 30% TEST ---
        # 1. Preparar los datos de prueba (30%) bajo las mismas variables seleccionadas
        X1_test = select(df_test, Not(ncol_datos))
        y1_test = categorical(String.(string.(df_test[!, ncol_datos])))
        y_test_final = coerce(y1_test, Multiclass)
        
        # 2. Reconstruir el dataset final óptimo para Train y Test
        X_final_train = res_gpn["e2"].df_final # El subconjunto optimizado
        
        # Extraer las mismas columnas seleccionadas en Test
        columnas_optimas = names(X_final_train)
        
        # Es necesario aplicar la transformación polinomial completa a Test 
        # y filtrar las mismas columnas que el algoritmo genético decidió usar
        X_pool_test = polexp(X1_test, grado=2)
        X_final_test = select(X_pool_test, columnas_optimas)
        
        # 3. Entrenar la máquina con el 70% entrenado y evaluar contra el 30% oculto
        mach_final = machine(modelo, X_final_train, y_train_final)
        fit!(mach_final, verbosity=0)
        
        # Evaluar predicciones directas sobre el conjunto Test en "Caja Negra"
        preds = predict_mode(mach_final, X_final_test)
        
        # Calcular métricas explícitas usando MLJ
        acc_val  = accuracy(preds, y_test_final)
        f1_val   = multiclass_f1score(preds, y_test_final)
        rec_val  = multiclass_recall(preds, y_test_final)
        prec_val = multiclass_precision(preds, y_test_final)
        
        # Almacenar en el contenedor seguro para hilos
        lista_resultados[i] = (
            acc_externo  = acc_val,
            f1_externo   = f1_val,
            rec_externo  = rec_val,
            prec_externo = prec_val,
        ...
        println("Thread $(threadid()): Corrida $i finalizada con Accuracy de Test = $(round(acc_val, digits=4))")
    end    
