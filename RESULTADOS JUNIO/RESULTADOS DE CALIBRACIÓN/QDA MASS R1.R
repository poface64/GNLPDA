...
# ==============================================================================
# FRAMEWORK QDA DE MASS PARALELIZADO CON EL ENFOQUE FUTURE (R)
# ==============================================================================
...
ruta_resultados  <- "C:\\Users\\Angel\\Desktop\\GNLPDA\\RESULTADOS JUNIO\\RESULTADOS DE CALIBRACIÓN\\RESULTADOS_QDA/"
...
archivos_csv <- archivos_csv[!grepl("_partitions", archivos_csv)][c(8)]
...
for (archivo in archivos_csv) {
  ...
  cat("\n==================================================================\n")
  cat("Iniciando Evaluación QDA (MASS) para:", nombre_base, "\n")
  cat("==================================================================\n")
  ...
  resultados_lista <- future_lapply(1:n_corridas, function(run) {
    ...
    # EVALUACIÓN DIRECTA (Método Analítico Cuadrático)
    tryCatch({
      modelo_final         <- MASS::qda(x = X_train_run, grouping = y_train_run)
  ...
  write.csv(df_res_instancia, 
            file = file.path(ruta_resultados, paste0(nombre_base, "_res_qda.csv")), 
            row.names = FALSE)
...
plan(sequential)
cat("\n==================================================================\n")
cat("Proceso completado. Clúster de 'future' liberado para QDA.\n")
cat("==================================================================\n")
...
# ==============================================================================
# SCRIPT DE DIAGNÓSTICO SECUENCIAL PARA QDA (MASS)
# ==============================================================================
...
# Obtener los nombres de los datasets originales
archivos_csv <- list.files(path = ruta_datasets, pattern = "\\.csv$", full.names = TRUE)
archivos_csv <- archivos_csv[!grepl("_partitions", archivos_csv)][8]
...
for (archivo in archivos_csv) {
  ...
  cat("\n==================================================================\n")
  cat("Iniciando Diagnóstico Secuencial QDA para:", nombre_base, "\n")
  cat("==================================================================\n")
  ...
  n_corridas <- 33
  resultados_lista <- vector("list", n_corridas)
  
  # BUCLE SECUENCIAL: Ejecución una a una para capturar anomalías
  for (run in 1:n_corridas) {
    ...
    # Inicialización local limpia (Evita el problema de los vínculos bloqueados)
    accuracy   <- 0
    kappa      <- 0
    macro_f1   <- 0
    macro_rec  <- 0
    ...
    tryCatch({
      ...
      # Blindaje de extracción de métricas
      ...
      # Mensaje de alerta inmediato en consola detallando el problema matemático
      cat("  [!] Anomalía matemática en Corrida:", run, "| Detalles:", e$message, "\n")
    })
    
    tiempo_total    <- proc.time() - tiempo_inicio
    tiempo_segundos <- as.numeric(tiempo_total["elapsed"])
    
    resultados_lista[[run]] <- data.frame(
      Corrida         = run,
      Accuracy        = accuracy,
      Kappa           = kappa,
      Macro_F1        = macro_f1,
  ...
  # Consolidar y guardar los resultados de la instancia
  df_res_instancia <- do.call(rbind, resultados_lista)
  write.csv(df_res_instancia, 
            file = file.path(ruta_resultados, paste0(nombre_base, "_res_qda.csv")), 
            row.names = FALSE)
  
  cat("Resultados guardados de forma segura para:", nombre_base, "\n")
}

cat("\n==================================================================\n")
cat("Análisis secuencial completado.\n")
cat("==================================================================\n")
