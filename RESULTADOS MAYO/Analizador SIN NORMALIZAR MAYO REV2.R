...
archivos_comp <- list.files(rutaRS, pattern = "Competidores_Final_MAYO", full.names = TRUE)
archivos_gnlp <- list.files(rutaRS, pattern = "^Competidores_Final_MAYO", full.names = TRUE)

# Crear Tabla Maestra de Accuracy
library(tidyverse)

# Hacer la super tabla comparativa
...
archivos_30 <- list.files(
  path = rutaRS,
  pattern = "^Resultados_Detalle_", # Ajusta el patrón según el nombre de tus archivos de 30 corridas
  full.names = TRUE
)
...
for (i in seq_along(rutasRS)) {
  ...
  dataset_name <- sub("Resultados_Detalle_", "", basename(rutasDS[i]))
...
write.csv(resumen_estadistico_gnlpda, 
          file.path(rutaRS, "Validacion_Estadistica_GNLPDA_MAYO.csv"), 
          row.names = FALSE)
...
for (i in seq_along(rutasRS)) {
  # Cargar las rutas 
  df_30 <- read.csv(rutasRS[i])
  # Separar el vector de accuracy's
  acc_vector <- df_30$Acc
  # Poner el nombre de el actual dataset
  ds_nombre_target <- basename(tabla_comparativa$Dataset[i])
  fila_ref <- tabla_comparativa[tabla_comparativa$Dataset == ds_nombre_target, ]
  # Resumen del nombre
  fila_resumen <- data.frame(Dataset = sub("\\.csv$", "", ds_nombre_target))
  # Aplicar para cada método sí se calcula el valor P y la media
  for (metodo in competidores_nombres) {
