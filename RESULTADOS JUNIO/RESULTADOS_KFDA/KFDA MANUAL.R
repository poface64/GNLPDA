...
kfda_entrenar <- function(K_train, y_train, r = NULL, reg = 1e-6) {
  ...
  # 2.- Calcular las matrices de dispersion
  for (cls in classes) {
  ...
  # 4. Coeficientes de la transformación (Dim: n_train x r)
  T_matrix <- solve(L) %*% eig$vectors[, 1:r, drop = FALSE]
  
  # 5. Proyectar y TRANSPONER inmediatamente para formato estándar
...
### KFDA para predecir ####
# Esta función asigna los puntos al centroide más cercano y devuelve las proyecciones
