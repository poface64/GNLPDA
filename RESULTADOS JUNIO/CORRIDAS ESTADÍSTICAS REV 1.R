...
########################
# =========================================================================
# CARGA Y PREPARACIÓN DE BASES PARA EL CASO i
# =========================================================================
i <- 1 

cat("Procesando Caso:", i, "\n")
cat("Dataset original:", basename(rutasDS[i]), "\n")
cat("Resultados GNLPDA:", basename(rutasRS[i]), "\n\n")

# Cargar base de datos original (Para LDA de MASS y KFDA)
...
# Cargar el detalle de las 30 corridas previas de GNLPDA
res_gnlpda <- read.csv(rutasRS[i])

# Generar la expansión polinomial completa del caso i (Matriz base para GNLPDA)
...
# =========================================================================
# CICLO DE EVALUACIÓN ESTADÍSTICA (30 SEMILLAS)
# =========================================================================

### Agregar la librería para SVM si no está cargada ###

# 1. Crear el contenedor expandido para las 30 precisiones
...
# Fijar las semillas para replicabilidad
semillas <- 1001:1030 
for (s in 1:30) {
  
  # 1. Construir el dataset GNLPDA específico para esta semilla (Fila s)
  df_gnlpda_optimo <- obtener_dataset_gnlpda(df_expandido_completo, res_gnlpda, idx_fila = s)
  
  # 2. Partición balanceada 70/30
  set.seed(semillas[s])
  indices_train <- createDataPartition(df_original$Clase, p = 0.70, list = FALSE)
  
  # Particiones para datos originales (LDA / KFDA)
  train_orig <- df_original[indices_train, ]
  test_orig  <- df_original[-indices_train, ]
  
  # Particiones para datos optimizados de la semilla s (GNLPDA)
  train_gnlpda <- df_gnlpda_optimo[indices_train, ]
  test_gnlpda  <- df_gnlpda_optimo[-indices_train, ]
  #plot(train_gnlpda[,-4],col = train_gnlpda$Clase)
  #library(rgl)
  #plot3d(train_gnlpda[,-4],col = as.numeric(train_gnlpda$Clase),
  #       size = 15)
  
  # =========================================================================
  # EVALUACIONES: MODELOS COMPETIDORES (Variables Originales)
  # =========================================================================
  
  # --- Método 1: LDA de MASS ---
  ...
  # --- Método 3: QDA (Protegido contra Incomputabilidad) ---
  # Nota técnica: QDA requiere que n > p dentro de cada clase. 
  # En el Caso 1 (150 obs, 3 vars) y Caso 2 (150 obs, 10 vars) correrá sin problemas.
  # En los Casos 3 a 6 (donde n = 15 y el split 70% deja aprox 3-4 obs por clase en train), 
  # QDA fallará si las variables exceden el tamaño de muestra de la clase.
  ...
  if (num_features < min_obs_clase) {
    ...
    resultados_particiones$QDA[s] <- mean(pred_qda == test_orig$Clase)
  } else {
    resultados_particiones$QDA[s] <- NA # Se marca como NA si no es computable numéricamente
  }
  
  # --- Método 4: SVM Radial ---
  # El parámetro gamma usa la lógica original: 1 / número de variables predictoras
  ...
  # --- Método 5: SVM Polinomial Grado 2 ---
  ...
  # --- Método 6: SVM Polinomial Grado 3 ---
  ...
  # =========================================================================
  # EVALUACIÓN: MÉTODO PROPUESTO (Subconjunto óptimo polinomial)
  # =========================================================================
  
  # --- Método 7: GNLPDA ---
  modelo_gnlpda <- lda(Clase ~ ., data = train_gnlpda)
  pred_gnlpda   <- predict(modelo_gnlpda, newdata = test_gnlpda)$class
  resultados_particiones$GNLPDA[s] <- mean(pred_gnlpda == test_gnlpda$Clase)
}
...
# =========================================================================
# 1. TRANSFORMACIÓN A FORMATO LARGO (Conservando la estructura de bloques)
# =========================================================================
df_long <- resultados_particiones %>% 
  # Se seleccionan todas las columnas de clasificadores omitiendo la columna de control 'Semilla'
  pivot_longer(
    cols = -Semilla, 
    names_to = "Metodo", 
    values_to = "Precision"
  ) %>% 
  # Convertir a factores las variables categóricas (Indispensable para las pruebas)
  mutate(
    Metodo = as.factor(Metodo),
    Semilla = as.factor(Semilla)
  ) %>% 
  # Opcional: Filtrar métodos incomputables si todo el vector es NA (como QDA en ciertos casos)
  filter(!is.na(Precision)) %>% 
  as.data.frame()

# =========================================================================
# 2. VISUALIZACIÓN (Gráfico de Cajas Cohesivo)
# =========================================================================
ggplot(df_long, aes(x = Metodo, y = Precision, fill = Metodo)) +
  geom_boxplot(alpha = 0.7) + 
  labs(
    title = paste0("Distribución de Precisión - Caso ", i),
    x = "Clasificador",
  ...
  theme(
    ...
    legend.position = "none" # Ocultar leyenda ya que el eje X tiene los nombres
  )

# =========================================================================
# 3. VERIFICACIÓN DE NORMALIDAD (Shapiro-Test)
# =========================================================================
cat("--- PRUEBAS DE NORMALIDAD (SHAPIRO-TEST) ---\n")
print(tapply(df_long$Precision, df_long$Metodo, shapiro.test))

# =========================================================================
# 4. PRUEBA MULTIVARIABLE RELACIONADA (Friedman Test)
# =========================================================================
# Estructura: Respuesta ~ Tratamiento | Bloque (Semilla)
...
# =========================================================================
# 5. COMPARACIONES MÚLTIPLES POST-HOC (Nemenyi Test)
# =========================================================================

if (prueba_friedman$p.value < 0.05) {
  cat("\nLa prueba de Friedman es significativa (p =", round(prueba_friedman$p.value, 5), ").\n")
  cat("Procediendo con la prueba post-hoc de Nemenyi...\n\n")
  
  # Ejecución exacta del test de Nemenyi para medidas repetidas
  prueba_nemenyi <- frdAllPairsNemenyiTest(
    y      = df_long$Precision, 
    groups = df_long$Metodo, 
    blocks = df_long$Semilla
  )
  
  cat("--- MATRIZ DE P-VALORES DE LA PRUEBA DE NEMENYI ---\n")
  print(summary(prueba_nemenyi))
  
} else {
  cat("\nNo se encontraron diferencias estadísticas significativas entre los métodos (p =", round(prueba_friedman$p.value, 5), ").\n")
}

########################
# Instalar la librería si no está en el entorno
# install.packages("tsutils")
library(tsutils)

# 1. Crear una matriz donde las columnas sean los clasificadores y las filas las 30 semillas
# (Friedman y Nemenyi trabajan sobre matrices de bloques de este tipo)
matriz_precisiones <- resultados_particiones %>% 
  select(-Semilla) %>% 
  # Opcional: Eliminar columnas si tienen NAs (como QDA) para evitar errores en el rango
  select(where(~ !all(is.na(.)))) %>% 
  as.matrix()

# 2. Generar el gráfico de Diferencia Crítica de Nemenyi directamente
# Al usar 'plot = TRUE', calcula los rangos medios internos, aplica el test 
# y dibuja las líneas de conexión automáticas de los grupos homólogos a un alpha = 0.05
png(filename = paste0("Caso_", i, "_CD_Diagram_Nemenyi.png"), 
    width = 7, height = 4, units = "in", res = 300)

nem <- nemenyi(matriz_precisiones, conf.level = 0.95, plottype = "mcb")
...
# 2.- Definir los objetos donde estan las rutas
### Rutas de los datos ###
rutaDS = "C:\\Users\\Angeal\\Desktop\\GNLPDA\\RESULTADOS JUNIO\\DATASETS/" # Windows
#rutaDS <- "~//Desktop/GNLPDA REV1/RESULTADOS ABRIL/DATASETS/" # UBUNTU
rutasDS <- list.files(
  path = rutaDS,
  full.names = TRUE);rutasDS
...
#################
# =========================================================================
# BOSQUEJO DE CARGA Y FILTRADO PARA EL CASO i
# =========================================================================

# 1. Definir el caso de estudio (por ejemplo, i = 1 para el Caso 1)
i <- 1 

# Imprimir en consola para verificar la alineación de archivos
cat("Procesando Caso:", i, "\n")
cat("Dataset original:", basename(rutasDS[i]), "\n")
cat("Resultados GNLPDA:", basename(rutasRS[i]), "\n\n")

# 2. Cargar la base de datos original (Para LDA de MASS y KFDA)
df_original <- read.csv(rutasDS[i])
df_original[,ncol(df_original)] = factor(df_original[,ncol(df_original)])
names(df_original)[ncol(df_original)] = "Clase"

# 3. Cargar el detalle de los resultados previos de GNLPDA
# (Se asume que este archivo contiene los índices o nombres de los atributos seleccionados)
res_gnlpda <- read.csv(rutasRS[i])

# 4. Generar la expansión polinomial completa del caso i
# Se usa la función guardada en el semáforo sobre las predictoras
df_expandido_completo <- data.frame(
  polexpj(df_original[, -ncol(df_original)]),
  Clase = df_original[, ncol(df_original)]
)
names(df_expandido_completo)

# 5. EXTRAER EL SUBCONJUNTO ÓPTIMO PARA GNLPDA
# NOTA: Aquí se debe adaptar según cómo guarde el AG los resultados.
# Supongamos que el archivo 'Resultados_Detalle_' tiene una columna llamada 'Variables_Optimas'
# o que contiene los nombres exactos de las columnas seleccionadas.

# Ejemplo A: Si el archivo es un vector o matriz de características seleccionadas:
vars_crudas <- gsub(" ", "", unlist(strsplit(res_gnlpda$Nombres_E2[i], ",")))
variables_seleccionadas = normalizar_nombres(vars_crudas)

# Construir el dataset exclusivo para el método propuesto
# Se toman solo las columnas elegidas por el AG y se le añade la variable objetivo
df_gnlpda_optimo <- data.frame(
  df_expandido_completo[, variables_seleccionadas, drop = FALSE],
  Clase = df_original[, ncol(df_original)]
)

##### Ciclo para las 30 semillas ####
library(caret) # Recomendada para particiones estratificadas fáciles

# Supongamos que ya estamos dentro del análisis del Caso i
# df_original y df_gnlpda_optimo ya han sido cargados y nombrados correctamente

# 1. Crear el contenedor para las 30 precisiones
resultados_particiones <- data.frame(
  Semilla  = 1:30,
  LDA_MASS = NA,
  KFDA     = NA,
...
# 2. Definir un vector con las 30 semillas fijas (pueden ser consecutivas o específicas)
semillas <- 1001:1030 
s = 1
# 3. Iniciar el ciclo de evaluación
for (s in 1:30) {
  
  # Fijar la semilla para asegurar la reproducibilidad de la partición 70/30
  set.seed(semillas[s])
  
  # Crear índices de entrenamiento estratificados basados en la columna de clase
  # (Garantiza que el 70% de cada uno de los 3 grupos vaya a Train)
  indices_train <- createDataPartition(df_original[, ncol(df_original)], p = 0.70, list = FALSE)
  
  # -------------------------------------------------------------------------
  # PARTICIÓNE DE DATOS ORIGINALES (Para LDA y KFDA)
  # -------------------------------------------------------------------------
  train_orig <- df_original[indices_train, ]
  test_orig  <- df_original[-indices_train, ]
  
  # -------------------------------------------------------------------------
  # PARTICIÓN DE DATOS OPTIMIZADOS (Para tu método GNLPDA)
  # -------------------------------------------------------------------------
  train_gnlpda <- df_gnlpda_optimo[indices_train, ]
  test_gnlpda  <- df_gnlpda_optimo[-indices_train, ]
  names(train_gnlpda)
  names(train_orig)
  # =========================================================================
  # EVALUACIÓN 1: LDA de MASS (Variables Originales)
  # =========================================================================
  ...
  # =========================================================================
  # EVALUACIÓN 2: KFDA (Variables Originales)
  # =========================================================================
  modelo_kfda <- kfda(train_orig, kernel.name = "rbfdot", kpar.sigma = 0.0001) # Checar si meterle irace
  pred_kfda = kfda.predict(modelo_kfda,test_orig)
  resultados_particiones$KFDA[s] <- mean(pred_kfda$class == test_orig$Clase)
  resultados_particiones$KFDA[s]

  # =========================================================================
  # EVALUACIÓN 3: GNLPDA (Subconjunto Polinomial Seleccionado)
  # =========================================================================
  # Evaluamos la separabilidad lineal del subconjunto usando un clasificador lineal (ej. lda)
  modelo_gnlpda <- lda(Clase ~ ., data = train_gnlpda)
  pred_gnlpda   <- predict(modelo_gnlpda, newdata = test_gnlpda)$class
  resultados_particiones$GNLPDA[s] <- mean(pred_gnlpda == test_gnlpda$Clase)
}
