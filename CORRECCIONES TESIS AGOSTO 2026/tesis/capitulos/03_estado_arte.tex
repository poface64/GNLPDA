...
Para mitigar esta dificultad, las variantes regularizadas de LDA incorporan términos de penalización o contracción sobre la matriz de dispersión, con el objetivo de mejorar su estabilidad numérica. En términos generales, estas estrategias modifican la estimación de la matriz de covarianza o añaden un término de regularización que favorece la inversión del sistema y reduce la sensibilidad del modelo ante fluctuaciones derivadas de muestras pequeñas. En este sentido, la regularización no elimina por completo los riesgos asociados al problema SSS, pero constituye una alternativa práctica para mejorar la estabilidad del discriminante lineal en  contextos de alta dimensionalidad \cite{Araveeporn2023}.


Otro enfoque reciente es el que propone Mahadi y colaboradores \cite{mahadi2024regularized} quienes propusiieron un enfoque denominado NL-RLDA, el cual se basa en tomar el discriminante lineal regularizado y cambiar la forma en que hace la estimación de la matriz inversa de covarianzas mediante una estimación de caracter no lineal. A diferencia de los esquemas lineales tradicionales de contracción, este método aplica dicha transformación no lineal sobre los eigenvalores de la matriz de 
covarianza, reduciendo la influencia de valores pequeños o nulos y mejorando la robustez del clasificador en escenarios donde la cantidad de variables es mayor a  la cantidad de observaciones ($p\geq n$). Los autores derivan un estimador consistende de la tasa de error de la clasificación y emplean una búsqueda de rejilla para seleccionar el parametro de regularización óptimo. Los resultados experimentales, tanto en datos sintéticos como reales, muestran que $NL-RLDA$ supera a las variantes lineales de RLDA y ofrece un desempeño competitivo frente a otros clasificadores modernos, consolidandose como una alternativa eficaz para los problemas de clasificación en alta dimensión.


\subsection{Two-Stage LDA}

Una estrategia alternativa frente a la singularidad en espacios de alta dimensionalidad consiste en utilizar esquemas de reducción de dimensionalidad en dos etapas. En la arquitectura comúnmente denominada PCA-LDA, primero se aplica Análisis de Componentes Principales para proyectar los datos hacia un subespacio de menor dimensión.
Esta etapa permite reducir la redundancia y mejorar el condicionamiento del problema antes de aplicar LDA \cite{agatra2025evaluating}.
