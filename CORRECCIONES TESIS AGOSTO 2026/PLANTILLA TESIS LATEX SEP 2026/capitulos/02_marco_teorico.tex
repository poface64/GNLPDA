...
En el desarrollo de sistemas de aprendizaje automático, el análisis exploratorio de los datos es una etapa fundamental para comprender la naturaleza del problema. Antes de entrenar modelos de clasificación complejos, la capacidad de observar geométricamente cómo se distribuyen las observaciones permite explorar patrones, detectar posibles valores atípicos e inspeccionar el grado de solapamiento entre las distintas clases.
\cite{oettl2024artificial,abukmeil2021survey}. 

\subsection{El desafío de visualizar datos de alta dimensión}

La cognición humana está intrínsecamente limitada a la percepción y comprensión de espacios de hasta tres dimensiones. Sin embargo, en áreas como la bioinformática, el procesamiento de imágenes o la radiómica, los conjuntos de datos suelen representarse en espacios de decenas, cientos o miles de características \cite{PENDIENTE_VisualizacionAltaDimension}. 

Intentar comprender la estructura de un conjunto de datos multidimensional a través del análisis aislado por pares de variables resulta ineficiente y propenso a omitir interacciones complejas que sean clave para entenderlos. A medida que la dimensionalidad aumenta, los datos se vuelven escasos en el espacio representativo, y las distancias euclidianas entre las observaciones pierden su capacidad para discriminar proximidades significativas, agravando los efectos de la maldición de la dimensionalidad \cite{PENDIENTE_MaldicionDimensionalidadDistancias}. Por lo tanto, surge la necesidad matemática de proyectar esta información hacia subespacios visualmente asimilables sin perder la estructura subyacente.

\subsection{Reducción dimensional como mecanismo de representación}

Para cerrar la brecha entre la alta dimensionalidad de los datos y las limitaciones de la percepción humana, se emplean técnicas de reducción dimensional orientadas a la visualización. El objetivo principal de estos mecanismos no es únicamente comprimir la información para reducir el costo computacional, sino encontrar un mapeo hacia un espacio de dimensión $d$ (donde $d \in \{2, 3\}$) que preserve las propiedades topológicas más relevantes del espacio original \cite{PENDIENTE_ReduccionDimensionalVisualizacion}.
...
Para efectos de visualización, la extracción de características suele ser preferida, ya que permite condensar la información de múltiples variables originales en unos pocos ejes proyectados, facilitando una vista panorámica del conjunto de datos.

\subsection{Proyecciones discriminantes para la visualización supervisada}

La utilidad de una visualización depende fuertemente del criterio matemático utilizado para proyectar los datos. El Análisis de Componentes Principales (PCA) es históricamente la técnica más utilizada para este propósito. Al proyectar las observaciones sobre sus dos o tres primeras componentes, se obtiene un mapa que preserva la mayor varianza global de los datos. Sin embargo, al ser un método no supervisado, PCA ignora por completo a qué clase pertenece cada observación \cite{PENDIENTE_LimitacionesPCAVisualizacion}.
...
Para ilustrar esta diferencia fundamental entre la maximización de la varianza global y la maximización de la separabilidad, la figura \ref{fig:MT1} presenta una comparativa geométrica conceptual. Mientras que PCA proyecta los datos buscando la dirección de mayor dispersión general (ignorando las etiquetas de clase), una proyección discriminante orienta los ejes específicamente para maximizar la distancia entre los centroides de las clases y minimizar el solapamiento interno.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=1\textwidth]{MT1.png}
	\caption{Comparativa entre el funcionamiento de PCA y LDA. Elaboración propia.}
	\label{fig:MT1}	
\end{figure}

\subsection{Visualización en dos y tres dimensiones}

La generación de gráficos de dispersión en dos y tres dimensiones (2D y 3D) a partir de proyecciones discriminantes constituye la herramienta visual más potente para evaluar cualitativamente un problema de clasificación. En un espacio proyectado de baja dimensión, una separación clara entre los grupos indica empíricamente que el problema puede ser resuelto con alta precisión \cite{PENDIENTE_InterpretacionScatterPlots}.

No obstante, en el contexto de la inteligencia artificial interpretable, la utilidad de estas representaciones 2D y 3D no radica únicamente en observar clases separadas, sino en la \textbf{trazabilidad} de sus ejes. Si un eje de visualización ($X$ o $Y$) se construye mediante una combinación matemática opaca (como ocurre en métodos de mapeo implícito o topológico), la visualización carece de explicabilidad. Por el contrario, cuando los ejes de proyección pueden rastrearse directamente hacia las variables originales o términos polinomiales explícitos, la representación geométrica se convierte en un instrumento auditable que explica qué factores detonan la separación de las clases. 

El Análisis Discriminante Lineal, que se detallará en la siguiente sección, proporciona precisamente la base matemática para construir estos ejes discriminantes trazables.
...
A diferencia del Análisis de Componentes Principales (PCA), que identifica direcciones de máxima varianza sin considerar las etiquetas de clase, LDA incorpora explícitamente la información supervisada del problema. Por ello, mientras PCA puede conservar direcciones con alta variabilidad global pero poca utilidad discriminante, LDA orienta la proyección hacia la separación entre grupos previamente definidos. En términos generales, esta separación se plantea como un compromiso entre maximizar la dispersión entre clases y minimizar la dispersión dentro de cada clase \cite{bishop2006pattern}.
...
%Para ilustrar esta diferencia fundamental entre la maximización de la %varianza global y la maximización de la separabilidad de clases, la Figura %\ref{fig:MT1} presenta una comparativa geométrica de lo que hace cada una. %Mientras que PCA proyecta los datos buscando la dirección de mayor %dispersión general (ignorando las etiquetas), LDA orienta la proyección %específicamente hacia el eje que maximiza la distancia entre los centroides %de las clases y minimiza el solapamiento interno.
