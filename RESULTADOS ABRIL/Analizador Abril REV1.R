...
for (i in seq_along(rutasDS)) {
  ...
  for (j in 1:nrow(datosRS)) {
    ...
    for(f in 1:10) {
      ...
      # 1. DATOS CRUDOS
      # =========================
      train_raw <- datosDS[train_idx, ]
      test_raw  <- datosDS[test_idx, ]
      
      y_train <- as.factor(train_raw[, ncol(train_raw)])
      y_test  <- as.factor(test_raw[, ncol(test_raw)])
      
      X_train <- train_raw[, -ncol(train_raw)]
      X_test  <- test_raw[, -ncol(test_raw)]
      
      # =========================
      # 2. ESCALADO (SOLO TRAIN)
      # =========================
      ...
      # 3. FILTRADO E1
      # =========================
      train_FIL <- as.data.frame(X_train_scaled)[, vars_e1_all, drop = FALSE]
      test_FIL  <- as.data.frame(X_test_scaled)[, vars_e1_all, drop = FALSE]
      
      # =========================
      # 4. EXPANSIÓN POLINOMIAL
      # =========================
      train_exp <- polexpj(train_FIL, 2)
      test_exp  <- polexpj(test_FIL, 2)
      
      # =========================
      # 5. SELECCIÓN E2
      # =========================
      train_eval <- train_exp[, vars_e2_ready, drop = FALSE]
      test_eval  <- test_exp[, vars_e2_ready, drop = FALSE]
      
      train_eval$Clase <- y_train
      test_eval$Clase  <- y_test
      
      # =========================
      # 6. MODELO
      # =========================
      fit_lda <- lda(Clase ~ ., data = train_eval)
      preds <- predict(fit_lda, test_eval)$class
      
      # =========================
      # 7. MÉTRICAS
      # =========================
      cm <- confusionMatrix(preds, test_eval$Clase)
      
      byClass <- cm$byClass
      if(is.null(dim(byClass))){
        byClass <- as.data.frame(t(byClass))
      }
